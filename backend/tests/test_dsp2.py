...
def _write_piecewise_wav(path: Path, segments, sr: int = 16000, amplitude: float = 0.5):
    ...
    for f_start, f_end, dur in segments:
        ...
        if f_start is None:
            ...
            # matching write_sine_wav's sampling of the instantaneous frequency.
...
def test_identical_clips_all_components_high(tmp_path):
    ...
    write_sine_wav(native_wav, freq_hz=130.0, freq_end_hz=170.0, duration_s=1.5)
    write_sine_wav(user_wav, freq_hz=130.0, freq_end_hz=170.0, duration_s=1.5)
...
def test_uniform_tempo_not_penalized(tmp_path):
    ...
    user_wav = tmp_path / "user.wav"
    write_sine_wav(native_wav, freq_hz=130.0, freq_end_hz=170.0, duration_s=1.5)
    write_sine_wav(user_wav, freq_hz=130.0, freq_end_hz=170.0, duration_s=1.725)  # 1.15x
...
def test_nonuniform_rhythm_penalized(tmp_path):
    ...
    write_sine_wav(uni_native, freq_hz=130.0, freq_end_hz=170.0, duration_s=1.5)
    write_sine_wav(uni_user, freq_hz=130.0, freq_end_hz=170.0, duration_s=1.725)
