...
HTTP, no DB — and prints a table of all four score components.
  ...
  python -m tools.calibrate                 # score table + gate check on the corpus
  python -m tools.calibrate --tune          # grid search + pitch-floor diagnostic
  python -m tools.calibrate --smoke         # synthetic end-to-end run, no manifest
  python -m tools.calibrate --manifest PATH # non-default manifest location
...
DEFAULT_MANIFEST = Path(__file__).resolve().parents[2] / "native_audio" / "manifest.json"
...
# dsp.ScoringParams field -> the dsp.py constant supplying its default, so
# --tune can print a recommendation in paste-able form.
CONSTANT_NAMES = {
    "k_pitch_semitones": "SCORE_K_PITCH_SEMITONES",
    "k_timing": "SCORE_K_TIMING",
    "k_energy_z": "SCORE_K_ENERGY_Z",
    "pitch_weight": "PITCH_WEIGHT",
    "timing_weight": "TIMING_WEIGHT",
    "energy_weight": "ENERGY_WEIGHT",
}

# --tune search space, keyed by the dsp.py constant each axis varies.
TUNE_GRID = {
...
def extract_corpus(entries: list, *, pitch_floor_hz: float = dsp.PITCH_FLOOR_HZ) -> dict:
    """(practice_id, kind) -> trimmed ProsodyFeatures for every corpus file."""
    return {
        (entry["practice_id"], kind): dsp.features_for(path, pitch_floor_hz=pitch_floor_hz)
...
def corpus_rows(
    ...
    feats: dict = None,
    *,
    params: dsp.ScoringParams = dsp.DEFAULT_SCORING,
    energy_lambda: float = dsp.DTW_ENERGY_LAMBDA,
) -> list:
    """Score every (take vs reference) pair; defaults are the shipped dsp constants."""
    ...
    for entry in entries:
        ...
        for kind in TAKE_KINDS:
            if kind not in entry["takes"]:
                continue
            aligned = dsp.align(
                feats[(pid, "reference")], feats[(pid, kind)], energy_lambda=energy_lambda
            )
            overall, pitch, timing, energy = dsp.score(aligned, params)
    ...
    for lam in TUNE_GRID["DTW_ENERGY_LAMBDA"]:
        ...
        for k_pitch, k_timing, k_energy in itertools.product(
            ...
            for w_pitch, w_timing, w_energy in WEIGHT_GRID:
                params = dsp.ScoringParams(
                    k_pitch_semitones=k_pitch,
                    k_timing=k_timing,
                    k_energy_z=k_energy,
                ...
                overalls = {key: dsp.score(a, params)[0] for key, a in aligned.items()}
                min_emu = min(
...
def recommended_constants(best: dict) -> dict:
    """The winning combo rendered as dsp.py constant assignments, so the printed
    recommendation can be pasted straight back into dsp.py."""
    return {
        "DTW_ENERGY_LAMBDA": best["energy_lambda"],
        **{
            CONSTANT_NAMES[field]: value
            for field, value in dataclasses.asdict(best["params"]).items()
        },
    }
...
def pitch_floor_sweep(entries: list) -> None:
    ...
    for floor in PITCH_FLOOR_SWEEP_HZ:
        # Only extraction depends on the floor; scoring uses the shipped constants.
        feats = extract_corpus(entries, pitch_floor_hz=floor)
...
def build_smoke_corpus(dirpath) -> list:
    ...
    def wav(name, f0, dur, f1=None):
        path = dirpath / name
        write_sine_wav(path, freq_hz=f0, duration_s=dur, freq_end_hz=f1)
        return path
...
def _run(entries: list, do_tune: bool, do_probe_mfcc: bool = False) -> int:
    ...
    if do_tune:
        ...
        for name, value in recommended_constants(best).items():
            print(f"    {name} = {value}")
        print("\nScores with recommended constants:")
        tuned_rows = corpus_rows(
            entries, feats, params=best["params"], energy_lambda=best["energy_lambda"]
        )
