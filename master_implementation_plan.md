...
- **2026-08-03 — Ambient-noise pipeline landed (ticket 17); spectral reduction gated on `MIN_PROFILE_SNR_DB = 6.0`.** Wheel pre-check passed: `scipy 1.18.0` + `noisereduce 3.0.3` and their full transitive closure resolve as prebuilt wheels under `--only-binary :all:` — but on **cp312**, the project venv's actual interpreter, not the cp314 the ticket assumed; `noisereduce` drags in matplotlib/pillow/fonttools (~50 MB) for a spectral-gating call that never plots. New `domain/dsp/noise.py` (`bandpass` / `snr_db` / `denoise` / `denoise_clip`), called from `worker_core.run` on the **user clip only**, after the bleed gate; `features_for` and the pure-DSP suite are untouched, as the ticket required. **Spec deviation, owner-approved:** applying `noisereduce` unconditionally with "the first 300ms" as the profile is unsafe when the user starts speaking immediately — the profile is then their own voice, and subtracting it stripped 82% of clip energy, dropping the identical-clip lifecycle score from ~100 to 89.8 (energy axis 67.4) and breaking the regression net. Measured separation is clean: a clip starting mid-speech reads ~0.17 dB, one with a genuine ambient lead-in reads 11–27 dB, so reduction now runs only above 6 dB and the bandpass is unconditional. The SNR is persisted on `audio_assets.snr_db` (column existed, never populated) **whether or not reduction ran** — it describes the recording, not the filtering. Note the synthetic corpus can only show denoising doesn't hard-fail, never that it helps: on tone+white-noise, bandpass-only outscores reduction at every noise level because spectral gating is tuned for speech. The STT content gate still sees the **raw** clip (ASR is trained on natural audio, and routing it through the cleaned signal would mean materializing a temp WAV). Full backend suite green (68 passed, 1 skipped).

- **2026-07-28 — Content axis added to scoring: a pronunciation sub-score from the STT gate's WER, blended at `CONTENT_WEIGHT = 0.15`.** The prosody scorer only measures *how* a line is said; this folds in *what* was said, reusing the WER the ticket-22 gate already computes against `Practice.transcript` (native and learner recite the same target, so "match the native's transcription" == "pronounce the target line intelligibly"). `dsp.content_score_from_wer` maps WER→0-100 linearly (100 at WER 0, 0 at WER≥1); `dsp.blend_content` computes `overall = (1-CW)*prosody + CW*content`, and **fails open** — no transcript or STT-unavailable → content is None → prosody-only, so the axis never penalizes a take it couldn't measure. Persisted as `prosody_jobs.content_score` (additive migration), exposed on `/jobs/{id}`, shown as a 4th sub-score in `Results.jsx`. **Weight is a placeholder** (0.15): measured on the corpus, WER is a blunt/noisy signal — it cleanly punishes mumbled/wrong-word takes (low_effort content 40/46, gibberish ~0) but does NOT separate a *clear* monotone from a *clear* expressive take (practice 2 emulation and monotone both WER 0.12), because it scores articulation, not intonation. Graduation gates re-checked on the blended corpus scores and still hold (p7 emulation 70.1 / margin 7.9; p2 emulation 75.3 / margin 3.3 — thinner, since content dilutes the prosody margin where content is equal). The prosody sub-weights (20/60/20, ADR 0003) are untouched; `CONTENT_WEIGHT` needs proper graduation on a larger set. Full backend + frontend suites green (pure `content_score_from_wer`/`blend_content` tests + a hermetic worker-wiring test).
- **2026-07-28 — Content gate v2 (ticket 22): STT replaces MFA likelihood; `CONTENT_GATE_MAX_WER` graduated to 0.80; ticket-20 gate retired.** The ticket-20 signal (`speech_log_likelihood` from *forced* alignment) could not discriminate content — gibberish overlapped genuine takes (root cause: forced alignment maps the given words onto any audio; the residual is the cross-speaker acoustic floor, the ADR 0003 wall). Replaced with actual recognition: `backend/content_gate.py` now shells `conda run -n stt python stt_runner.py <wav>` (faster-whisper `base`, quarantined in a new user-scope `stt` conda env — no cp314 wheel, wheel rule honored, `requirements.txt` unchanged) and rejects on word-error rate vs the practice transcript. Same contract as before: pure (`normalize_transcript`/`word_error_rate`/`decide`) + orchestration (`assess`) split, **fails open** on any STT infra error, same "we couldn't make out the line" reject; `worker_core.run` call site unchanged. Recognition cost dropped from ~45s/job (MFA model load) to ~2–5s/clip. **Calibrated** on the owner's set (six correct takes + `gibberish{1..4}`): correct WER 0.13–0.60, gibberish WER 1.00–1.40 — a clean [0.60, 1.00] gap; `CONTENT_GATE_MAX_WER = 0.80` sits mid-gap, biased high so real accented learners (worse than the low-effort take at 0.60) aren't wrongly rejected. All six correct PASS, all four gibberish FAIL. Whisper hallucination on gibberish was the flagged risk — confirmed benign: it invents *different* words (e.g. gibberish4 → English "Condor and crashes echoing…"), never the target line, so WER stays ≥ 1.0. MFA constants / `parse_analysis_csv` / the `RUN_MFA_TESTS` test removed; STT integration test opt-in via `RUN_STT_TESTS=1`. Full backend suite green (pure WER/decision tests included).
